...
@Component(
	immediate = true,
	property = {
		"clay.data.provider.key=" + AvailableShipmentsTable.NAME,
		"clay.data.set.display.name=" + AvailableShipmentsTable.NAME
	},
	service = {
...
public class AvailableShipmentsTable
	extends BaseTableClayDataSetDisplayView
	implements ClayDataSetActionProvider, ClayDataSetDataProvider<Shipment> {

	public static final String NAME = "commerce-processing-shipments";
		...
		return DropdownItemListBuilder.add(
			dropdownItem -> {
				...
				dropdownItem.setLabel(
					LanguageUtil.get(httpServletRequest, "plan"));
			}
		).build();
	}
	...
	public List<Shipment> getItems(
		...
		return _getProcessingShipment(httpServletRequest);
	}
	...
	public int getItemsCount(
		...
		return _getProcessingShipment(
			httpServletRequest
		).size();
	}

	List<Shipment> _getProcessingShipment(
		...
		return _shipmentService.getCommerceShipments(
			...
			shipment ->
				(CommerceShipmentConstants.SHIPMENT_STATUS_PROCESSING ==
					shipment.getStatus()) &&
				(Validator.isNull(shipment.getExpectedDate()) ||
				 Validator.isNull(shipment.getShippingDate()))
		).map(
			commerceShipment -> {
				try {
